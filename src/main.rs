...
//	Load the window UI
	uiapp.connect_activate( |app| {
		...
		let close_button: gtk::ModelButton = builder.get_object("close_button").expect("Couldn't get close_button");

        let location_chooser = gtk::ComboBoxText::new();
        location_chooser.append(Some("national"), "National");
        ...
        location_chooser.set_active_id(Some("national"));
        ...
        national();
        let national_gif = gtk::Image::new_from_file("/tmp/national.gif");
        ...
        map_stack.add_named(&national_gif, "national");

        map_stack.set_visible_child(&national_gif);
        ...
        location_chooser.connect_changed(clone!(@weak map_stack => move |location_chooser| {
            ...
            } else if location_chooser.get_active_id().unwrap() == "national" {
                map_stack.set_visible_child(&national_gif);
            }
        }));
...
//        win.set_resizable(false);
        ...
        close_button.connect_clicked(clone!(@weak win => move |_| {
        	win.destroy();
        }));
...
fn national() {
    std::fs::remove_file("/tmp/national.gif");
	let mut file = std::fs::File::create("/tmp/national.gif").expect("create failed");

    let mut gif_r = ureq::get("https://radar.weather.gov/ridge/Conus/Loop/NatLoop_Small.gif").call().into_reader();
